...
import { apiGenerate, apiEditScript, apiGetLatestGeneration, apiSaveScreenplay } from '../api'
...
export default function StoryStudio() {
    ...
    const generationId = activeProject.generationId ?? null
    ...
    const isNewGeneration = location.state?.newGeneration === true || skipAutoLoad

    const [story, setStory] = useState('')
    const [phase, setPhase] = useState('input') // input → generating → studio
    ...
    const handleSave = async () => {
        ...
        try {
            await apiSaveScreenplay(generationId, script)
            setSaveSuccess(true)
            notify('✅ Screenplay saved!')
            setTimeout(() => setSaveSuccess(false), 2500)
        } catch (err) {
            console.error('Save failed:', err)
            notify('❌ Failed to save screenplay')
        } finally {
            setSaving(false)
        }
    }
    ...
    const downloadPDF = () => {
        ...
        doc.text(`Generated by CineForge AI — ${new Date().toLocaleDateString()}`, margin, y)
        y += 10
        ...
        doc.setFontSize(10)
        doc.setTextColor(30)
        const lines = doc.splitTextToSize(script, pageWidth)
        ...
        doc.save(`${projectTitle.replace(/\s+/g, '_')}_screenplay.pdf`)
    ...
    // ── Download as DOCX ──────────────────────────────────────
    const downloadDOCX = () => {
        const rtfContent = `{\\rtf1\\ansi{\\fonttbl{\\f0 Courier New;}{\\f1 Helvetica;}}
{\\f1\\fs36\\b ${projectTitle.replace(/\\/g, '\\\\')}}\\par
{\\f1\\fs16\\i Generated by CineForge AI}\\par
{\\f0\\fs20\\par}
${script.split('\n').map(line => line.replace(/\\/g, '\\\\').replace(/{/g, '\\{').replace(/}/g, '\\}') + '\\par').join('\n')}
        ...
        a.href = url
        a.download = `${projectTitle.replace(/\s+/g, '_')}_screenplay.doc`
    ...
    useEffect(() => {
        ...
        if (isNewGeneration) return  // location.state says fresh

        // If a specific generation was passed via navigation state (from history Load/Open)
        const loadGen = location.state?.loadGeneration
        if (loadGen) {
            setGeneration(loadGen)
            setPhase('studio')
            setStory(loadGen.story_input || '')
            setWordCount((loadGen.screenplay || '').split(/\s+/).filter(Boolean).length)
            // Clear the location state to avoid reloading on re-renders
                ...
                <div style={{ background: '#FFFFFF', borderBottom: '1px solid rgba(160,110,70,0.12)', padding: '0 28px', display: 'flex', alignItems: 'center', flexShrink: 0, position: 'sticky', top: 0, zIndex: 30 }}>
                    ...
                    {activeTab === 'script' && (
                        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 5, padding: '8px 0' }}>
                            ...
                            <button className="toolbar-btn" onClick={handleSave} disabled={saving || !generationId}
                                style={{
                                    padding: '5px 10px', borderRadius: 6, fontSize: 11.5,
                                    color: saveSuccess ? '#2D8B5E' : '#C07840', fontWeight: 600,
                                    display: 'flex', alignItems: 'center', gap: 4,
                                    opacity: (!generationId || saving) ? 0.5 : 1,
                                    cursor: (!generationId || saving) ? 'not-allowed' : 'pointer',
                                    transition: 'color 0.2s',
                                }}>
                                {saving ? <Loader2 size={12} style={{ animation: 'spin 1s linear infinite' }} /> : saveSuccess ? <Check size={12} /> : <Save size={12} />}
                                {saving ? 'Saving…' : saveSuccess ? 'Saved' : 'Save'}
                            </button>
                            <div style={{ position: 'relative' }}>
                                <button className="toolbar-btn" onClick={() => setShowDownload(!showDownload)}
                                    style={{ padding: '5px 8px', borderRadius: 6, display: 'flex', alignItems: 'center', gap: 3, cursor: 'pointer' }}>
                                    <Download size={12} /><ChevronDown size={9} />
                                </button>
                                {showDownload && (
                                    <div style={{
                                        position: 'absolute', top: '100%', right: 0, marginTop: 4,
                                        background: '#FFFFFF', border: '1px solid rgba(160,110,70,0.2)', borderRadius: 8,
                                        overflow: 'hidden', zIndex: 100, minWidth: 150,
                                        boxShadow: '0 8px 28px rgba(100,60,20,0.12)',
                                    }}>
                                        <button onClick={downloadPDF}
                                            style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '9px 14px', background: 'transparent', border: 'none', color: '#5A3E2A', fontSize: 12.5, textAlign: 'left', cursor: 'pointer' }}
                                            onMouseEnter={(e) => { e.currentTarget.style.background = '#FAF7F2' }}
                                            onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent' }}>
                                            <FileDown size={13} color="#C07840" />
                                            Download as PDF
                                        </button>
                                        <button onClick={downloadDOCX}
                                            style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '9px 14px', background: 'transparent', border: 'none', color: '#5A3E2A', fontSize: 12.5, textAlign: 'left', cursor: 'pointer', borderTop: '1px solid rgba(160,110,70,0.08)' }}
                                            onMouseEnter={(e) => { e.currentTarget.style.background = '#FAF7F2' }}
                                            onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent' }}>
                                            <FileText size={13} color="#C07840" />
                                            Download as DOCX
                                        </button>
                                    </div>
                                )}
